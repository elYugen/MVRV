...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [newsResponse, heroesResponse, usersResponse, streamersResponse] = await Promise.all(
          [
  ...
  const latestStreamers = streamers.slice(0, 3);
    ...
    <>
      ...
      <section>
        <div className={adminstyles.container}>
          ...
          {/* ----- Les 3 derniers streamers ajoutés ----- */}
          <div className={styles.newsContainer}>
            {streamers.length === 0 ? (
              <div className={styles.newsContainer}>
                <div className={styles.newsError}>
                  <p>Il n'y a aucun streamers actuellement</p>
                </div>
              </div>
            ) : (
              <table className={herolist.herolistTable}>
                <thead>
                  <tr>
                    <th colSpan="2"><center>Les 3 derniers streamers ajoutées</center></th>
                  </tr>
                </thead>
                <tbody>
                <tr>
                <td className={herolist.heroImage}>
                  {latestStreamers.map((streamer) => (
                        <img key={streamer._id} src={streamer.picture} alt={streamer.name} />
                  ))}
                </td>
                </tr>
                </tbody>
            ...
            <div className={styles.newsLatest}>
              <button className={adminstyles.UserButton}>
                <Link to="/admin/streamer" className={adminstyles.viewMoreButton}>
                  Voir les autres streamers →
                </Link>
              </button>
            </div>
          </div>
