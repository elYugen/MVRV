...
function CreateModal({ closeModal, onCreate }) {
    ...
    const handleChange = (e) => {
        ...
        if (name.startsWith('passive_')) {
            ...
            setHero(prevHero => {
                const updatedPassives = [...prevHero.passives];
                updatedPassives[idx] = { ...updatedPassives[idx], [field]: value };
                return { ...prevHero, passives: updatedPassives };
            });
    ...
    const handleAddPassive = () => {
        setHero(prevHero => ({
            ...prevHero,
            passives: [...prevHero.passives, { name: '', description: '' }]
        }));
    };
    ...
    const handleRemovePassive = (index) => {
        setHero(prevHero => ({
            ...prevHero,
            passives: prevHero.passives.filter((_, i) => i !== index)
        }));
    };
    ...
    const handleSave = () => {
        if (!hero.name || !hero.tier || !hero.health || !hero.difficulty || !hero.category) {
